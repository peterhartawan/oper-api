...
class ReportingEmployee implements FromView
{
    ...
    public function view(): View
    {
        ...
        $templatetask2 = $order2->select('order.idorder',
                    DB::raw("DATE_FORMAT(order.booking_time, '%d-%M-%Y ') as date"), 
                    DB::raw("DATE_FORMAT(order.booking_time, '%H:%i:%s ') as time"),
                    ...
                    // DB::raw('CONCAT(order.origin_name, "-", order.destination_name) AS route'),
                    // 'usr.name as name_dispatcher', 'driver.name as name_driver', 
                    DB::raw("DATE_FORMAT(order.dispatch_at, '%H:%i:%s ') as dispatch_time") , 
                    'order.dispatch_at','order.booking_time',  'order.updated_at', 
                    'booking_time', 'employee.name',
                    'order_type.name as name_ordertype')
                    ->leftjoin('users as driver','driver.id','=','order.driver_userid')
                    ->leftjoin('users as employee','employee.id','=','order.employee_userid')
