...
CREATE TABLE IF NOT EXISTS `oper`.`order` (
  ...
  INDEX `fk_order_order_type1_idx` (`order_type_idorder_type` ASC),
  ...
  CONSTRAINT `fk_order_order_type1`
    ...
    REFERENCES `mydb`.`order_type` (`idorder_type`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
CREATE TABLE IF NOT EXISTS `oper`.`task` (
  ...
  `is_required` VARCHAR(45) NULL,
  `is_need_photo` VARCHAR(45) NULL,
  `is_need_inspector_validation` VARCHAR(45) NULL,
  `inspector_idinspector` INT NULL,
  PRIMARY KEY (`idtask`),
  INDEX `task_task_template_id_foreign` (`task_template_id` ASC),
  INDEX `fk_task_inspector1_idx` (`inspector_idinspector` ASC),
  CONSTRAINT `task_task_template_id_foreign`
    FOREIGN KEY (`task_template_id`)
    REFERENCES `oper`.`task_template` (`task_template_id`),
  CONSTRAINT `fk_task_inspector1`
